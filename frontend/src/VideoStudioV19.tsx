...
export default function VideoStudioV19() {
  ...
  const [box, setBox] = useState<TrackingBoxV17>({ x: .34, y: .20, width: .24, height: .34 })
  ...
  const [cameraMotion, setCameraMotion] = useState<CameraMotionResultV19 | null>(null)
  ...
  const persistAssets = (next: TrackAsset[]) => {
    setAssets(next)
    try { localStorage.setItem(ASSET_KEY, JSON.stringify(next.slice(0, 16))) } catch { /* local quota is non-fatal */ }
  }
  ...
  const useSource = async (next: File, label?: string) => {
    ...
    setFile(next); setUrl(objectUrl); setSourceLabel(label || next.name); setCurrentTime(0); setTrack(null); setCameraMotion(null); setAnalysis(null); setTranscript(null); setError(null)
    if (resultUrl) URL.revokeObjectURL(resultUrl)
    setResultUrl(null); setResultBlob(null); setBusy('inspect')
    try {
      const info = await inspectSourceV15(next)
      setInspection(info); setTrackStart(0); setTrackEnd(Math.min(30, info.duration))
  ...
  const beginBox = (event: ReactPointerEvent<HTMLDivElement>, mode: 'move' | 'resize') => {
    ...
    setTrack(null); setBox(initial)
    const move = (pointer: PointerEvent) => {
      const dx = (pointer.clientX - startX) / Math.max(1, rect.width)
      const dy = (pointer.clientY - startY) / Math.max(1, rect.height)
      if (mode === 'move') setBox({ ...initial, x: clamp(initial.x + dx, 0, 1 - initial.width), y: clamp(initial.y + dy, 0, 1 - initial.height) })
      else setBox({ ...initial, width: clamp(initial.width + dx, .04, 1 - initial.x), height: clamp(initial.height + dy, .04, 1 - initial.y) })
    }
  ...
  const runTrack = async () => {
    ...
    try {
      ...
      setTrack(await adaptiveTrackV19(file, box, trackStart, end, clamp(currentTime, trackStart, end), trackFps, searchRadius))
  ...
  const saveAsset = () => {
    if (!track) return
    const next: TrackAsset = { id: crypto.randomUUID(), name: assetName.trim() || `Track ${assets.length + 1}`, color: assetColor, sourceLabel, createdAt: new Date().toISOString(), track }
  ...
  const analyzeCamera = async () => {
    ...
    try { setCameraMotion(await cameraMotionV19(file)) }
  ...
  const downloadSrt = () => {
    ...
    const link = document.createElement('a'); link.href = href; link.download = 'MAGHRABI-v19-arabic-captions.srt'; link.click(); URL.revokeObjectURL(href)
  }
  ...
  return <main className="min-h-screen bg-[#05080d] text-slate-100" dir="rtl">
    ...
    <div className="mx-auto grid max-w-[1750px] gap-4 p-4 xl:grid-cols-[350px_minmax(0,1fr)_390px]">
      <aside className="space-y-4">
        <section className="rounded-3xl border border-white/8 bg-[#0a1019] p-4"><div className="flex items-center gap-2"><Crosshair className="h-4 w-4 text-cyan-300"/><h2 className="text-xs font-black">SCALE‑ADAPTIVE TRACKER</h2></div><p className="mt-2 text-[9px] leading-5 text-slate-500">حرّك الصندوق فوق الهدف. V19 يتعامل مع تغيّر الحجم ويجمّد آخر Pose موثوق أثناء Occlusion القصير ثم يوسّع Search Radius تلقائيًا.</p><div className="mt-3 grid grid-cols-2 gap-2 text-[9px]"><label>START<input type="number" step=".1" value={trackStart} onChange={e=>setTrackStart(Number(e.target.value))} className="mt-1 w-full rounded-lg bg-black/30 p-2"/></label><label>END<input type="number" step=".1" value={trackEnd} onChange={e=>setTrackEnd(Number(e.target.value))} className="mt-1 w-full rounded-lg bg-black/30 p-2"/></label><label>FPS<input type="number" min="2" max="8" value={trackFps} onChange={e=>setTrackFps(Number(e.target.value))} className="mt-1 w-full rounded-lg bg-black/30 p-2"/></label><label>SEARCH<input type="number" min=".03" max=".2" step=".01" value={searchRadius} onChange={e=>setSearchRadius(Number(e.target.value))} className="mt-1 w-full rounded-lg bg-black/30 p-2"/></label></div><button disabled={!file||!!busy} onClick={runTrack} className="mt-3 w-full rounded-xl bg-cyan-300 px-3 py-2 text-xs font-black text-slate-950">{busy==='track'?'TRACKING…':'ADAPTIVE TRACK'}</button>{track&&<div className="mt-3 rounded-xl bg-black/25 p-3 text-[9px]"><p>Confidence <b>{Math.round(track.averageConfidence*100)}%</b> · Low <b>{track.lowConfidencePoints}</b></p><p className="mt-1">Occlusion points <b>{track.occlusionPoints}</b> · Scale range <b>{track.scaleChange.toFixed(2)}×</b></p><div className="mt-2 flex h-3 overflow-hidden rounded bg-white/5">{track.points.slice(0,150).map((point,index)=><div key={index} className={point.occluded?'bg-rose-400':point.confidence<.38?'bg-amber-300':'bg-emerald-300'} style={{flex:1}} title={`${timeLabel(point.time)} · ${Math.round(point.confidence*100)}%`}/>)}</div></div>}</section>

        <section className="rounded-3xl border border-white/8 bg-[#0a1019] p-4"><div className="flex items-center gap-2"><Save className="h-4 w-4 text-fuchsia-300"/><h2 className="text-xs font-black">TRACKING ASSETS</h2></div><div className="mt-3 grid grid-cols-[1fr_48px] gap-2"><input value={assetName} onChange={e=>setAssetName(e.target.value)} className="rounded-lg bg-black/30 p-2 text-[10px]" placeholder="اسم الهدف"/><input type="color" value={assetColor} onChange={e=>setAssetColor(e.target.value)} className="h-9 w-full rounded-lg bg-black/30 p-1"/></div><button disabled={!track} onClick={saveAsset} className="mt-2 w-full rounded-xl border border-fuchsia-300/30 px-3 py-2 text-[10px] font-black text-fuchsia-200">SAVE TRACK ASSET</button><div className="mt-3 max-h-56 space-y-2 overflow-auto">{assets.map(asset=><div key={asset.id} className="flex items-center gap-2 rounded-xl border border-white/8 bg-black/20 p-2"><span className="h-3 w-3 rounded-full" style={{background:asset.color}}/><button onClick={()=>{setTrack(asset.track);setBox(asset.track.points[0]||box);setAssetName(asset.name);setAssetColor(asset.color)}} className="min-w-0 flex-1 text-right"><p className="truncate text-[9px] font-black">{asset.name}</p><p className="truncate text-[8px] text-slate-600">{asset.sourceLabel} · {Math.round(asset.track.averageConfidence*100)}%</p></button><button onClick={()=>persistAssets(assets.filter(item=>item.id!==asset.id))} className="rounded-lg p-1 text-rose-300"><Trash2 className="h-3.5 w-3.5"/></button></div>)}{!assets.length&&<p className="text-[9px] text-slate-600">لا توجد Tracking Assets محفوظة بعد.</p>}</div></section>
      </aside>

      <section className="space-y-4">
        <div className="rounded-3xl border border-white/8 bg-[#070b11] p-4"><div className="mb-3 flex items-center justify-between text-[10px]"><span className="font-black text-slate-400">PRODUCTION MONITOR</span><span className="font-mono text-cyan-200">{timeLabel(currentTime)} / {timeLabel(duration)}</span></div><div ref={monitorRef} className="relative mx-auto max-h-[70vh] overflow-hidden rounded-2xl bg-black" style={{aspectRatio}}>{url?<video ref={videoRef} src={url} controls className="h-full w-full object-contain" onTimeUpdate={e=>setCurrentTime(e.currentTarget.currentTime)}/>:<div className="grid h-full min-h-[430px] place-items-center text-center text-slate-600"><div><Film className="mx-auto h-12 w-12"/><p className="mt-3 text-sm font-bold">افتح فيديو أو Latest Edit</p></div></div>}{file&&<div onPointerDown={e=>beginBox(e,'move')} className="absolute cursor-move border-2 border-cyan-300 bg-cyan-300/5" style={{left:`${displayedBox.x*100}%`,top:`${displayedBox.y*100}%`,width:`${displayedBox.width*100}%`,height:`${displayedBox.height*100}%`}}><span className="absolute -top-5 right-0 rounded bg-cyan-300 px-1.5 py-.5 text-[8px] font-black text-slate-950">ADAPTIVE TARGET</span><button onPointerDown={e=>beginBox(e,'resize')} className="absolute -bottom-2 -left-2 h-4 w-4 rounded-full border-2 border-slate-950 bg-cyan-300"/></div>}</div><div className="mt-3 flex flex-wrap gap-2 text-[9px] text-slate-500"><span>{sourceLabel||'No source'}</span>{inspection&&<><span>· {inspection.color.width}×{inspection.color.height}</span><span>· {inspection.hasAudio?'AUDIO':'NO AUDIO'}</span><span className={isHdr?'text-amber-300':''}>· {isHdr?'HDR — finish in V15 first':'SDR'}</span></>}</div></div>
        ...
        {analysis&&<div className="rounded-3xl border border-white/8 bg-[#0a1019] p-4"><div className="flex items-center justify-between"><h3 className="text-xs font-black">AUTO CUT MAP</h3><span className="text-[9px] text-slate-500">{analysis.sceneCount} scenes · {analysis.analyzedCount} scored</span></div><div className="mt-3 flex h-12 overflow-hidden rounded-xl border border-white/8 bg-black/30">{analysis.cutList.map((scene,index)=><button key={index} onClick={()=>{if(videoRef.current)videoRef.current.currentTime=scene.start}} className="border-l border-black/40 bg-slate-700/70 text-[7px] font-black hover:bg-cyan-300 hover:text-slate-950" style={{flex:Math.max(.25,scene.duration)}} title={`${timeLabel(scene.start)}–${timeLabel(scene.end)}`}>{index+1}</button>)}</div><div className="mt-4 grid gap-2 md:grid-cols-4">{analysis.highlights.slice(0,8).map((scene,index)=><button key={index} onClick={()=>{if(videoRef.current)videoRef.current.currentTime=scene.start}} className="rounded-xl border border-amber-300/20 bg-amber-300/5 p-2 text-right"><p className="text-[8px] font-black text-amber-200">HIGHLIGHT {index+1} · {Math.round((scene.highlightScore||0)*100)}%</p><p className="mt-1 text-[8px] text-slate-500">{timeLabel(scene.start)} · {scene.duration.toFixed(1)}s</p></button>)}</div></div>}
      </section>

      <aside className="space-y-4">
        <section className="rounded-3xl border border-white/8 bg-[#0a1019] p-4"><div className="flex items-center gap-2"><Camera className="h-4 w-4 text-emerald-300"/><h2 className="text-xs font-black">CAMERA MOTION / STABILIZE</h2></div><button disabled={!file||!!busy} onClick={analyzeCamera} className="mt-3 w-full rounded-xl border border-emerald-300/30 px-3 py-2 text-[10px] font-black text-emerald-200">{busy==='camera'?'ANALYZING…':'ANALYZE CAMERA MOTION'}</button>{cameraMotion&&<div className="mt-3 rounded-xl bg-black/25 p-3 text-[9px]"><p className="font-black text-emerald-200">{cameraMotion.classification.toUpperCase()}</p><p className="mt-1">Stability <b>{Math.round(cameraMotion.stability*100)}%</b> · Jitter <b>{cameraMotion.jitter.toFixed(4)}</b></p><div className="mt-2 flex h-16 items-end gap-px overflow-hidden">{cameraMotion.samples.slice(0,90).map((sample,index)=><div key={index} className="flex-1 bg-emerald-300/60" style={{height:`${clamp(sample.magnitude*900,2,100)}%`}}/>)}</div></div>}<label className="mt-3 block text-[9px]">STABILIZE STRENGTH <span className="float-left">{Math.round(stabilizeStrength*100)}%</span><input type="range" min=".1" max="1" step=".05" value={stabilizeStrength} onChange={e=>setStabilizeStrength(Number(e.target.value))} className="mt-1 w-full accent-emerald-300"/></label><button disabled={!file||!!busy||isHdr} onClick={stabilize} className="mt-2 w-full rounded-xl bg-emerald-300 px-3 py-2 text-xs font-black text-slate-950">STABILIZE VIDEO</button>{isHdr&&<p className="mt-2 text-[8px] text-amber-300">HDR: حوّل المصدر بأمان في V15 قبل Stabilization.</p>}</section>

        <section className="rounded-3xl border border-white/8 bg-[#0a1019] p-4"><div className="flex items-center gap-2"><Highlighter className="h-4 w-4 text-amber-300"/><h2 className="text-xs font-black">AUTO CUT / HIGHLIGHTS</h2></div><label className="mt-3 block text-[9px]">SCENE THRESHOLD <span className="float-left">{sceneThreshold.toFixed(2)}</span><input type="range" min=".12" max=".7" step=".01" value={sceneThreshold} onChange={e=>setSceneThreshold(Number(e.target.value))} className="mt-1 w-full accent-amber-300"/></label><button disabled={!file||!!busy} onClick={analyzeProduction} className="mt-2 w-full rounded-xl border border-amber-300/30 px-3 py-2 text-[10px] font-black text-amber-200">{busy==='production'?'SCORING…':'ANALYZE CUTS + HIGHLIGHTS'}</button><label className="mt-3 block text-[9px]">HIGHLIGHT REEL MAX <span className="float-left">{highlightDuration}s</span><input type="range" min="10" max="90" step="5" value={highlightDuration} onChange={e=>setHighlightDuration(Number(e.target.value))} className="mt-1 w-full accent-amber-300"/></label><button disabled={!analysis||!!busy||isHdr} onClick={makeHighlight} className="mt-2 w-full rounded-xl bg-amber-300 px-3 py-2 text-xs font-black text-slate-950">{busy==='highlight'?'RENDERING…':'BUILD HIGHLIGHT REEL'}</button></section>

        <section className="rounded-3xl border border-white/8 bg-[#0a1019] p-4"><div className="flex items-center gap-2"><Subtitles className="h-4 w-4 text-fuchsia-300"/><h2 className="text-xs font-black">FASTER‑WHISPER WORKER</h2></div><div className={`mt-3 rounded-xl border p-3 text-[9px] ${whisper?.configured?'border-emerald-300/20 bg-emerald-300/5 text-emerald-200':'border-amber-300/20 bg-amber-300/5 text-amber-200'}`}>{whisper?.configured?'STT_WORKER_URL configured':'غير مهيأ — أضف STT_WORKER_URL لخدمة Faster‑Whisper مستقلة.'}</div><label className="mt-3 block text-[9px]">LANGUAGE<input value={language} onChange={e=>setLanguage(e.target.value)} className="mt-1 w-full rounded-lg bg-black/30 p-2" placeholder="ar"/></label><button disabled={!file||!whisper?.configured||!!busy} onClick={transcribe} className="mt-2 w-full rounded-xl bg-fuchsia-300 px-3 py-2 text-xs font-black text-slate-950">{busy==='stt'?'TRANSCRIBING…':'AUTO TRANSCRIBE'}</button>{transcript&&<div className="mt-3 rounded-xl bg-black/25 p-3 text-[9px]"><p className="max-h-24 overflow-auto text-slate-300">{String(transcript.text||'')||`${transcript.segments?.length||0} timed segments`}</p>{transcript.segments?.length?<button onClick={downloadSrt} className="mt-2 rounded-lg border border-fuchsia-300/30 px-2 py-1 text-fuchsia-200">DOWNLOAD ARABIC SRT</button>:null}</div>}</section>

        <section className="rounded-3xl border border-white/8 bg-[#0a1019] p-4"><div className="flex items-center gap-2"><Sparkles className="h-4 w-4 text-cyan-300"/><h2 className="text-xs font-black">V19 PIPELINE</h2></div><div className="mt-3 space-y-2 text-[9px] leading-5 text-slate-500"><p>Detection في V18 → Adaptive/Occlusion Track هنا → Save Track Asset → Camera Analysis/Stabilize → Auto Cut/Highlight → Faster‑Whisper → V15 Final Master.</p></div><div className="mt-3 grid grid-cols-2 gap-2"><a href="#video-v18" className="rounded-xl border border-white/10 px-2 py-2 text-center text-[9px] font-black">V18 DETECT</a><a href="#video-v15" className="rounded-xl border border-white/10 px-2 py-2 text-center text-[9px] font-black"><WandSparkles className="ml-1 inline h-3 w-3"/>V15 MASTER</a></div></section>
      </aside>
    </div>
