...
export default function VideoStudioV5() {
  ...
  const pipDragRef = useRef<{ id: string; clientX: number; clientY: number; x: number; y: number; width: number; height: number; scale: number } | null>(null)
  const transformDragRef = useRef<{ clientX: number; clientY: number; panX: number; panY: number; width: number; height: number } | null>(null)
  ...
  const addPipVideo = async (event: ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const startAt = projectTime
      const track: PipTrack = {
        id: uid(), fileIndex, startAt, endAt: startAt + Math.min(duration, 6), sourceStart: 0,
        sourceEnd: Math.min(duration, 6), scale: .30, opacity: 1, x: .66, y: .62, borderRadius: .08,
      }
      setProject((state) => ({ ...state, videoOverlays: [...state.videoOverlays, track] }))
      setSelection({ kind: 'pip', id: track.id }); setError(null)
    } catch (e) { setError(e instanceof Error ? e.message : 'تعذر إضافة Picture-in-Picture.') }
    event.target.value = ''
  }
  ...
  const insertFreezeFrame = () => {
    if (!selectedClip) return
    const at = clamp(previewTime, selectedClip.start, selectedClip.end - .02)
    const leftOk = at - selectedClip.start > .05
    const rightOk = selectedClip.end - at > .05
    const left: Clip = { ...selectedClip, id: uid(), end: at, freezeFrame: false }
    const freeze: Clip = { ...selectedClip, id: uid(), start: at, end: Math.min(selectedClip.end, at + .06), speed: 1, speedRamp: 'off', reverse: false, freezeFrame: true, freezeDuration: 2 }
    const right: Clip = { ...selectedClip, id: uid(), start: Math.min(selectedClip.end, at + .06), freezeFrame: false }
    const replacement = [leftOk ? left : null, freeze, rightOk ? right : null].filter(Boolean) as Clip[]
  ...
  const startRecorder = async () => {
    try {
      ...
      recorder.onstop = async () => {
        setRecording(false); const blob = new Blob(chunksRef.current, { type: 'video/webm' })
  ...
  const restoreProject = async () => {
    try {
      ...
      const restored = { ...initialProject, ...snap.project, videoOverlays: snap.project.videoOverlays || [] }
  ...
  const exportProject = async () => {
    ...
    try {
      const blob = await renderVideoProjectV5(videos.map((item) => item.file), audios.map((item) => item.file), images.map((item) => item.file), {
    ...
    } catch (e) { setError(e instanceof Error ? e.message : 'تعذر تصدير V5.') } finally { setBusy(false) }
  }
  ...
  const beginPipDrag = (event: ReactPointerEvent<HTMLDivElement>, track: PipTrack) => {
    ...
    pipDragRef.current = { id: track.id, clientX: event.clientX, clientY: event.clientY, x: track.x, y: track.y, width: rect.width, height: rect.height, scale: track.scale }
  ...
  const movePip = (event: ReactPointerEvent<HTMLDivElement>) => {
    ...
    updatePip(drag.id, { x: clamp(drag.x + (event.clientX - drag.clientX) / usableX, 0, 1), y: clamp(drag.y + (event.clientY - drag.clientY) / usableY, 0, 1) })
  }
  const endPipDrag = () => { pipDragRef.current = null }

  const beginTransformDrag = (event: ReactPointerEvent<HTMLDivElement>) => {
    if (!transformDrag || !selectedClip) return
    const rect = event.currentTarget.getBoundingClientRect(); event.currentTarget.setPointerCapture(event.pointerId)
    transformDragRef.current = { clientX: event.clientX, clientY: event.clientY, panX: selectedClip.panXStart || 0, panY: selectedClip.panYStart || 0, width: rect.width, height: rect.height }
  }
  const moveTransform = (event: ReactPointerEvent<HTMLDivElement>) => {
    ...
    const x = clamp(drag.panX + ((event.clientX - drag.clientX) / drag.width) * 2, -1, 1)
    const y = clamp(drag.panY + ((event.clientY - drag.clientY) / drag.height) * 2, -1, 1)
    updateClip(selectedClip.id, { panXStart: x, panXEnd: x, panYStart: y, panYEnd: y })
  }
    ...
    <main className="min-h-screen bg-[#050710] text-slate-100">
      <div className="relative mx-auto max-w-[1920px] px-3 py-3 md:px-5">
        <header className="flex flex-wrap items-center justify-between gap-3 border-b border-white/10 pb-3">
          <div className="flex items-center gap-3"><div className="grid h-11 w-11 place-items-center rounded-2xl bg-violet-400/10"><Film className="h-5 w-5 text-violet-200" /></div><div><div className="flex items-center gap-2"><h1 className="text-lg font-black">MAGHRABI Video Studio</h1><span className="rounded-full border border-cyan-300/20 bg-cyan-300/[.06] px-2 py-1 text-[9px] font-black text-cyan-200">CREATOR V5</span></div><p className="mt-1 text-[10px] text-slate-500">PiP · Audio Ducking · Reverse · Freeze · Privacy FX · Direct Transform</p></div></div>
          <div className="flex flex-wrap gap-2"><button onClick={saveProject} className="rounded-xl border border-white/10 px-3 py-2 text-[10px] font-black"><Save className="mr-1 inline h-3.5 w-3.5" />حفظ</button><button onClick={restoreProject} className="rounded-xl border border-white/10 px-3 py-2 text-[10px] font-black">استعادة</button><a href="#video-v4" className="rounded-xl border border-white/10 px-3 py-2 text-[10px] font-black text-slate-400">V4</a><select value={outputSize} onChange={(e) => setOutputSize(e.target.value as OutputSize)} className="rounded-xl border border-white/10 bg-[#0b111d] px-3 text-[10px]"><option value="720p">720p</option><option value="1080p">1080p</option><option value="portrait">9:16</option><option value="square">1:1</option></select><select value={quality} onChange={(e) => setQuality(e.target.value as RenderQuality)} className="rounded-xl border border-white/10 bg-[#0b111d] px-3 text-[10px]"><option value="draft">Draft</option><option value="standard">Standard</option><option value="high">High</option></select><button onClick={exportProject} disabled={busy || !project.clips.length} className="rounded-xl bg-gradient-to-r from-violet-500 to-cyan-500 px-4 py-2 text-xs font-black disabled:opacity-30">{busy ? 'Rendering...' : 'EXPORT V5'}</button></div>
        </header>

        <div className="mt-3 flex flex-wrap gap-1 rounded-2xl border border-white/10 bg-white/[.025] p-1.5">
          {(['media','transitions','effects','audio','tools'] as Panel[]).map((item) => <button key={item} onClick={() => setPanel(item)} className={`rounded-xl px-4 py-2 text-[10px] font-black ${panel === item ? 'bg-white text-black' : 'text-slate-400'}`}>{item === 'media' ? 'MEDIA' : item === 'transitions' ? 'TRANSITIONS' : item === 'effects' ? 'EFFECTS / PRIVACY' : item === 'audio' ? 'AUDIO' : 'SMART TOOLS'}</button>)}
        </div>

        <section className="mt-3 grid gap-3 2xl:grid-cols-[280px_minmax(0,1fr)_350px]">
          <aside className="rounded-3xl border border-white/10 bg-[#090e19] p-4">
            ...
            {panel === 'effects' && <><p className="text-[10px] font-black tracking-widest text-slate-500">EFFECTS / PRIVACY</p>{selectedClip ? <div className="mt-4 space-y-4"><div className="grid grid-cols-2 gap-2">{filters.map((item)=><button key={item.value} onClick={()=>updateClip(selectedClip.id,{filter:item.value})} className={`rounded-xl border px-3 py-2 text-[9px] font-black ${selectedClip.filter===item.value?'border-cyan-300/40 bg-cyan-300/10':'border-white/10'}`}>{item.label}</button>)}</div><label className="block text-[9px] text-slate-500">PRIVACY EFFECT<select className={fieldClass()} value={selectedClip.privacyEffect||'none'} onChange={(e)=>updateClip(selectedClip.id,{privacyEffect:e.target.value as PrivacyEffect})}><option value="none">None</option><option value="blur">Blur Region</option><option value="mosaic">Mosaic Region</option></select></label>{selectedClip.privacyEffect&&selectedClip.privacyEffect!=='none'&&<div className="grid grid-cols-2 gap-2"><label className="text-[9px] text-slate-600">X<input className={fieldClass()} type="number" min="0" max=".95" step=".01" value={selectedClip.privacyX||.35} onChange={(e)=>updateClip(selectedClip.id,{privacyX:clamp(Number(e.target.value),0,.95)})}/></label><label className="text-[9px] text-slate-600">Y<input className={fieldClass()} type="number" min="0" max=".95" step=".01" value={selectedClip.privacyY||.3} onChange={(e)=>updateClip(selectedClip.id,{privacyY:clamp(Number(e.target.value),0,.95)})}/></label><label className="text-[9px] text-slate-600">WIDTH<input className={fieldClass()} type="number" min=".05" max="1" step=".01" value={selectedClip.privacyWidth||.3} onChange={(e)=>updateClip(selectedClip.id,{privacyWidth:clamp(Number(e.target.value),.05,1)})}/></label><label className="text-[9px] text-slate-600">HEIGHT<input className={fieldClass()} type="number" min=".05" max="1" step=".01" value={selectedClip.privacyHeight||.22} onChange={(e)=>updateClip(selectedClip.id,{privacyHeight:clamp(Number(e.target.value),.05,1)})}/></label><label className="col-span-2 text-[9px] text-slate-600">INTENSITY<input type="range" min=".05" max="1" step=".05" value={selectedClip.privacyIntensity||.55} onChange={(e)=>updateClip(selectedClip.id,{privacyIntensity:Number(e.target.value)})} className="mt-2 w-full accent-fuchsia-300"/></label></div>}</div> : <p className="mt-4 text-[10px] text-slate-600">حدد Clip لتعديل المؤثرات.</p>}</>}
            {panel === 'audio' && <><p className="text-[10px] font-black tracking-widest text-slate-500">AUDIO MIXER</p><label className="mt-4 flex items-center justify-between rounded-xl border border-white/10 p-3 text-[10px] font-black"><span><Volume2 className="mr-2 inline h-4 w-4"/>Audio Ducking</span><input type="checkbox" checked={project.audioDuckingEnabled} onChange={(e)=>setProject(state=>({...state,audioDuckingEnabled:e.target.checked}))}/></label><label className="mt-4 block text-[9px] text-slate-500">DUCKING STRENGTH · {Math.round(project.duckingStrength*100)}%<input type="range" min="0" max="1" step=".05" value={project.duckingStrength} onChange={(e)=>setProject(state=>({...state,duckingStrength:Number(e.target.value)}))} className="mt-2 w-full accent-cyan-300"/></label><p className="mt-3 text-[9px] leading-5 text-slate-600">عند تفعيل Ducking تنخفض الموسيقى تلقائيًا عندما يحتوي صوت الفيديو الأساسي على كلام أو صوت بارز.</p>{selectedAudio&&<div className="mt-5 rounded-2xl border border-cyan-300/10 p-3"><p className="text-[10px] font-black">المسار المحدد</p><label className="mt-3 block text-[9px] text-slate-600">VOLUME<input className={fieldClass()} type="number" min="0" max="2" step=".05" value={selectedAudio.volume} onChange={(e)=>updateAudio(selectedAudio.id,{volume:clamp(Number(e.target.value),0,2)})}/></label></div>}</>}
            {panel === 'tools' && <><p className="text-[10px] font-black tracking-widest text-slate-500">SMART / CLIP TOOLS</p><div className="mt-4 grid grid-cols-2 gap-2"><button disabled={!selectedClip} onClick={()=>selectedClip&&updateClip(selectedClip.id,{reverse:!selectedClip.reverse})} className={`rounded-xl border p-3 text-[9px] font-black disabled:opacity-30 ${selectedClip?.reverse?'border-violet-300/40 bg-violet-300/10':'border-white/10'}`}><ArrowLeftRight className="mx-auto mb-2 h-4 w-4"/>REVERSE</button><button disabled={!selectedClip} onClick={insertFreezeFrame} className="rounded-xl border border-sky-300/20 p-3 text-[9px] font-black disabled:opacity-30"><Square className="mx-auto mb-2 h-4 w-4"/>FREEZE HERE</button></div><button onClick={runSilenceDetection} disabled={!selectedClip||silenceBusy} className="mt-3 flex w-full items-center justify-center gap-2 rounded-xl bg-cyan-400/10 px-3 py-3 text-[10px] font-black text-cyan-100 disabled:opacity-30"><Mic2 className="h-4 w-4" />{silenceBusy?'تحليل الصمت...':'Silence Detection'}</button>{silence&&<div className="mt-3 rounded-2xl border border-cyan-300/15 bg-cyan-300/[.04] p-3"><p className="text-[10px] font-black">تم اكتشاف {silence.intervals.length} مناطق صامتة</p><p className="mt-1 text-[9px] text-slate-500">إجمالي {fmt(silence.totalSilence)} من {fmt(silence.duration)}</p><button onClick={removeDetectedSilence} className="mt-3 w-full rounded-xl bg-white px-3 py-2 text-[10px] font-black text-black">إزالة الصمت تلقائيًا</button></div>}<div className="mt-5"><p className="text-[10px] font-black text-slate-500">SPEED RAMP</p><div className="mt-2 space-y-2">{speedRamps.map((item)=><button key={item.value} disabled={!selectedClip} onClick={()=>selectedClip&&updateClip(selectedClip.id,{speedRamp:item.value})} className={`w-full rounded-xl border p-3 text-left ${selectedClip?.speedRamp===item.value?'border-fuchsia-300/40 bg-fuchsia-300/10':'border-white/10'}`}><p className="text-[10px] font-black">{item.label}</p><p className="mt-1 text-[9px] text-slate-600">{item.description}</p></button>)}</div></div></>}
          </aside>

          <div className="min-w-0 space-y-3">
            <div className="rounded-3xl border border-white/10 bg-[#080d17] p-4"><div className="mb-3 flex flex-wrap items-center justify-between gap-2"><div><p className="text-xs font-black">PROGRAM MONITOR</p><p className="mt-1 text-[9px] text-slate-600">Project {fmt(projectTime)}</p></div><button disabled={!selectedClip} onClick={()=>setTransformDrag(v=>!v)} className={`rounded-xl border px-3 py-2 text-[9px] font-black disabled:opacity-30 ${transformDrag?'border-emerald-300/40 bg-emerald-300/10 text-emerald-100':'border-white/10 text-slate-400'}`}><Move className="mr-1 inline h-3.5 w-3.5"/>Direct Transform</button></div><div className="relative mx-auto aspect-video overflow-hidden rounded-2xl border border-white/10 bg-black" onPointerDown={beginTransformDrag} onPointerMove={moveTransform} onPointerUp={endTransform} onPointerCancel={endTransform}>{selectedVideo&&selectedClip?<><video ref={videoRef} src={selectedVideo.url} className="h-full w-full object-contain" style={{filter:previewCss,transform:`scale(${currentZoom}) translate(${currentPanX*16}%,${currentPanY*16}%)`,transformOrigin:'center'}} onTimeUpdate={onTimeUpdate} onPlay={()=>setPlaying(true)} onPause={()=>setPlaying(false)} playsInline/>{selectedClip.privacyEffect&&selectedClip.privacyEffect!=='none'&&<div className="pointer-events-none absolute border border-fuchsia-300/60 bg-black/10" style={{left:`${(selectedClip.privacyX||.35)*100}%`,top:`${(selectedClip.privacyY||.3)*100}%`,width:`${(selectedClip.privacyWidth||.3)*100}%`,height:`${(selectedClip.privacyHeight||.22)*100}%`,backdropFilter:selectedClip.privacyEffect==='blur'?`blur(${4+(selectedClip.privacyIntensity||.55)*16}px)`:'blur(2px)'}}><span className="absolute left-1 top-1 rounded bg-black/70 px-1.5 py-1 text-[8px] font-black text-fuchsia-200">{selectedClip.privacyEffect.toUpperCase()}</span></div>}{activePips.map((track)=>{const asset=videos[track.fileIndex];if(!asset)return null;return <div key={track.id} data-id={track.id} onPointerDown={(e)=>beginPipDrag(e,track)} onPointerMove={movePip} onPointerUp={endPipDrag} onPointerCancel={endPipDrag} className={`absolute cursor-move overflow-hidden border ${selection?.kind==='pip'&&selection.id===track.id?'border-emerald-300 ring-2 ring-emerald-300/20':'border-white/30'}`} style={{width:`${track.scale*100}%`,left:`${track.x*(1-track.scale)*100}%`,top:`${track.y*(1-track.scale)*100}%`,opacity:track.opacity,borderRadius:`${(track.borderRadius||0)*100}px`}}><video src={asset.url} muted autoPlay={playing} loop playsInline className="block h-auto w-full"/><span className="pointer-events-none absolute left-1 top-1 rounded bg-black/60 px-1.5 py-1 text-[8px] font-black text-emerald-200">PiP</span></div>})}{activeImages.map((track)=>{const asset=images[track.fileIndex];return asset?<img key={track.id} src={asset.url} className="pointer-events-none absolute bottom-5 right-5 max-h-[35%] max-w-[35%] object-contain" style={{opacity:track.opacity}}/>:null})}{activeTitles.map((track)=><div key={track.id} className="pointer-events-none absolute inset-x-8 top-1/2 -translate-y-1/2 text-center font-black text-white"><span className="rounded-lg bg-black/45 px-3 py-1.5" style={{fontSize:clamp(track.size/2,16,54)}}>{track.text}</span></div>)}{activeSubs.map((track)=><div key={track.id} className="pointer-events-none absolute inset-x-8 bottom-[6%] text-center font-bold" style={{color:track.color,fontSize:clamp(track.size/2,14,40)}}><span className="rounded-md bg-black/50 px-3 py-1.5">{track.text}</span></div>)}</>:<div className="grid h-full place-items-center"><Film className="h-12 w-12 text-slate-800"/></div>}</div><div className="mt-3 flex flex-wrap items-center justify-center gap-2"><button onClick={togglePlay} disabled={!selectedClip} className="rounded-xl bg-white px-4 py-2 text-xs font-black text-black disabled:opacity-30">{playing?<Pause className="mr-1 inline h-4 w-4"/>:<Play className="mr-1 inline h-4 w-4"/>}{playing?'إيقاف':'تشغيل'}</button><button onClick={splitClip} disabled={!selectedClip} className="rounded-xl border border-cyan-300/20 px-4 py-2 text-xs font-black"><Scissors className="mr-1 inline h-4 w-4"/>Split</button><button onClick={deleteSelection} disabled={!selection} className="rounded-xl border border-rose-300/20 px-3 py-2 text-rose-200 disabled:opacity-30"><Trash2 className="h-4 w-4"/></button>{selectedClip?.reverse&&<span className="rounded-lg border border-violet-300/20 bg-violet-300/10 px-2 py-1 text-[9px] font-black text-violet-200">REVERSE · Render</span>}{selectedClip?.freezeFrame&&<span className="rounded-lg border border-sky-300/20 bg-sky-300/10 px-2 py-1 text-[9px] font-black text-sky-200">FREEZE {selectedClip.freezeDuration}s</span>}</div></div>

            <div className="rounded-3xl border border-white/10 bg-[#080d17] p-4"><div className="mb-3 flex items-center justify-between"><div><p className="text-xs font-black">MULTI-TRACK TIMELINE</p><p className="mt-1 text-[9px] text-slate-600">Video · PiP · Title · Subtitle · Music · Overlay</p></div><div className="flex items-center gap-2"><ZoomOut className="h-4 w-4 text-slate-600"/><input type="range" min="3" max="24" value={timelineZoom} onChange={(e)=>setTimelineZoom(Number(e.target.value))} className="w-28 accent-cyan-300"/><ZoomIn className="h-4 w-4 text-slate-600"/></div></div><div className="overflow-x-auto rounded-2xl border border-white/8 bg-black/20"><div className="relative" style={{width:timelineWidth}}><div className="pointer-events-none absolute bottom-0 top-0 z-20 w-px bg-red-400" style={{left:80+projectTime*timelineZoom}}/>{['VIDEO','PIP VIDEO','TITLE','SUBTITLE','MUSIC','OVERLAY'].map((label)=><div key={label} className="h-16 border-b border-white/5"><div className="sticky left-0 z-30 flex h-full w-20 items-center bg-[#080d17] px-2 text-[9px] font-black text-slate-600">{label}</div></div>)}<div className="absolute left-20 top-0 h-16">{project.clips.map((clip,index)=>{const off=clipOffsets[index];if(!off)return null;const keyframed=Math.abs((clip.zoomEnd||1)-(clip.zoomStart||1))>.001||Math.abs((clip.panXEnd||0)-(clip.panXStart||0))>.001||Math.abs((clip.panYEnd||0)-(clip.panYStart||0))>.001;return <button key={clip.id} onClick={()=>{setSelection({kind:'clip',id:clip.id});setProjectTime(off.start)}} className={`absolute top-2 h-12 rounded-xl border px-3 text-left ${selection?.kind==='clip'&&selection.id===clip.id?'border-violet-200 bg-violet-400/20':'border-violet-300/15 bg-violet-400/10'}`} style={{left:off.start*timelineZoom,width:Math.max(75,off.duration*timelineZoom)}}><span className="text-[9px] font-black">CLIP {index+1}</span>{clip.speedRamp&&clip.speedRamp!=='off'&&<span className="ml-2 text-[8px] text-fuchsia-300">⚡</span>}{clip.reverse&&<span className="ml-2 text-[8px] text-violet-200">↔</span>}{clip.freezeFrame&&<span className="ml-2 text-[8px] text-sky-200">▣</span>}{keyframed&&<><span className="absolute bottom-1 left-1 text-[8px] text-amber-300">◆</span><span className="absolute bottom-1 right-1 text-[8px] text-amber-300">◆</span></>}</button>})}</div><div className="absolute left-20 top-16 h-16">{project.videoOverlays.map((track)=><button key={track.id} onClick={()=>setSelection({kind:'pip',id:track.id})} className="absolute top-2 h-12 rounded-xl border border-emerald-300/20 bg-emerald-400/10 px-3 text-[9px] font-black text-emerald-100" style={{left:track.startAt*timelineZoom,width:Math.max(75,(track.endAt-track.startAt)*timelineZoom)}}>PiP VIDEO</button>)}</div><div className="absolute left-20 top-32 h-16">{project.textTracks.map((track)=><button key={track.id} onClick={()=>setSelection({kind:'title',id:track.id})} className="absolute top-2 h-12 rounded-xl border border-sky-300/15 bg-sky-400/10 px-3 text-[9px]" style={{left:track.startAt*timelineZoom,width:Math.max(70,(track.endAt-track.startAt)*timelineZoom)}}>{track.text}</button>)}</div><div className="absolute left-20 top-48 h-16">{project.subtitleTracks.map((track)=><button key={track.id} onClick={()=>setSelection({kind:'subtitle',id:track.id})} className="absolute top-2 h-12 rounded-xl border border-amber-300/15 bg-amber-400/10 px-3 text-[9px]" style={{left:track.startAt*timelineZoom,width:Math.max(70,(track.endAt-track.startAt)*timelineZoom)}}>{track.text}</button>)}</div><div className="absolute left-20 top-64 h-16">{project.audioTracks.map((track)=><button key={track.id} onClick={()=>setSelection({kind:'audio',id:track.id})} className="absolute top-2 h-12 rounded-xl border border-cyan-300/15 bg-cyan-400/10 px-3 text-[9px]" style={{left:track.startAt*timelineZoom,width:Math.max(70,(track.sourceEnd-track.sourceStart)*timelineZoom)}}>MUSIC</button>)}</div><div className="absolute left-20 top-80 h-16">{project.imageTracks.map((track)=><button key={track.id} onClick={()=>setSelection({kind:'image',id:track.id})} className="absolute top-2 h-12 rounded-xl border border-fuchsia-300/15 bg-fuchsia-400/10 px-3 text-[9px]" style={{left:track.startAt*timelineZoom,width:Math.max(70,(track.endAt-track.startAt)*timelineZoom)}}>OVERLAY</button>)}</div></div></div></div>

            {resultUrl&&<div className="rounded-3xl border border-emerald-300/15 bg-emerald-300/[.035] p-5"><div className="flex items-center justify-between"><p className="text-sm font-black text-emerald-100">اكتمل Render V5</p><Sparkles className="h-5 w-5 text-emerald-300"/></div><video controls src={resultUrl} className="mt-4 max-h-[520px] w-full rounded-2xl bg-black"/><a href={resultUrl} download="MAGHRABI-video-v5.mp4" className="mt-4 flex items-center justify-center gap-2 rounded-xl bg-white px-4 py-3 text-sm font-black text-black"><Download className="h-4 w-4"/>تنزيل الفيديو</a></div>}
          </div>

          <aside className="rounded-3xl border border-white/10 bg-[#090e19] p-4"><div className="flex items-center justify-between"><div><p className="text-[10px] font-black tracking-widest text-slate-500">INSPECTOR</p><p className="mt-1 text-xs font-black">{selection?.kind.toUpperCase()||'PROJECT'}</p></div><WandSparkles className="h-4 w-4 text-violet-300"/></div>{selectedClip&&selectedVideo&&<div className="mt-5 space-y-4"><div className="grid grid-cols-2 gap-2"><label className="text-[9px] text-slate-600">START<input className={fieldClass()} type="number" step=".05" value={selectedClip.start} onChange={(e)=>updateClip(selectedClip.id,{start:clamp(Number(e.target.value),0,selectedClip.end-.05)})}/></label><label className="text-[9px] text-slate-600">END<input className={fieldClass()} type="number" step=".05" value={selectedClip.end} onChange={(e)=>updateClip(selectedClip.id,{end:clamp(Number(e.target.value),selectedClip.start+.05,selectedVideo.duration)})}/></label></div>{selectedClip.freezeFrame&&<label className="block text-[9px] text-slate-600">FREEZE DURATION<input className={fieldClass()} type="number" min=".2" max="12" step=".1" value={selectedClip.freezeDuration||2} onChange={(e)=>updateClip(selectedClip.id,{freezeDuration:clamp(Number(e.target.value),.2,12)})}/></label>}<div className="grid grid-cols-2 gap-2"><label className="text-[9px] text-slate-600">BASE SPEED<input className={fieldClass()} type="number" min=".25" max="4" step=".05" value={selectedClip.speed} onChange={(e)=>updateClip(selectedClip.id,{speed:clamp(Number(e.target.value),.25,4)})}/></label><label className="text-[9px] text-slate-600">VOLUME<input className={fieldClass()} type="number" min="0" max="2" step=".05" value={selectedClip.volume} onChange={(e)=>updateClip(selectedClip.id,{volume:clamp(Number(e.target.value),0,2)})}/></label></div><div><p className="text-[9px] font-black text-slate-500">KEYFRAME TRANSFORM</p><div className="mt-2 grid grid-cols-2 gap-2"><label className="text-[9px] text-slate-600">ZOOM START<input className={fieldClass()} type="number" min="1" max="4" step=".05" value={selectedClip.zoomStart||1} onChange={(e)=>updateClip(selectedClip.id,{zoomStart:clamp(Number(e.target.value),1,4)})}/></label><label className="text-[9px] text-slate-600">ZOOM END<input className={fieldClass()} type="number" min="1" max="4" step=".05" value={selectedClip.zoomEnd||1} onChange={(e)=>updateClip(selectedClip.id,{zoomEnd:clamp(Number(e.target.value),1,4)})}/></label><label className="text-[9px] text-slate-600">PAN X END<input className={fieldClass()} type="number" min="-1" max="1" step=".05" value={selectedClip.panXEnd||0} onChange={(e)=>updateClip(selectedClip.id,{panXEnd:clamp(Number(e.target.value),-1,1)})}/></label><label className="text-[9px] text-slate-600">PAN Y END<input className={fieldClass()} type="number" min="-1" max="1" step=".05" value={selectedClip.panYEnd||0} onChange={(e)=>updateClip(selectedClip.id,{panYEnd:clamp(Number(e.target.value),-1,1)})}/></label></div></div><label className="flex items-center justify-between rounded-xl border border-white/10 px-3 py-2 text-[10px]"><span>Green Screen / Chroma</span><input type="checkbox" checked={!!selectedClip.chromaEnabled} onChange={(e)=>updateClip(selectedClip.id,{chromaEnabled:e.target.checked})}/></label></div>}{selectedPip&&<div className="mt-5 space-y-4"><p className="text-[10px] font-black text-emerald-200">PICTURE-IN-PICTURE</p><div className="grid grid-cols-2 gap-2"><label className="text-[9px] text-slate-600">START<input className={fieldClass()} type="number" min="0" step=".1" value={selectedPip.startAt} onChange={(e)=>updatePip(selectedPip.id,{startAt:Math.max(0,Number(e.target.value))})}/></label><label className="text-[9px] text-slate-600">END<input className={fieldClass()} type="number" min=".1" step=".1" value={selectedPip.endAt} onChange={(e)=>updatePip(selectedPip.id,{endAt:Math.max(selectedPip.startAt+.1,Number(e.target.value))})}/></label><label className="text-[9px] text-slate-600">SCALE<input className={fieldClass()} type="number" min=".08" max=".85" step=".02" value={selectedPip.scale} onChange={(e)=>updatePip(selectedPip.id,{scale:clamp(Number(e.target.value),.08,.85)})}/></label><label className="text-[9px] text-slate-600">OPACITY<input className={fieldClass()} type="number" min="0" max="1" step=".05" value={selectedPip.opacity} onChange={(e)=>updatePip(selectedPip.id,{opacity:clamp(Number(e.target.value),0,1)})}/></label><label className="text-[9px] text-slate-600">X<input className={fieldClass()} type="number" min="0" max="1" step=".02" value={selectedPip.x} onChange={(e)=>updatePip(selectedPip.id,{x:clamp(Number(e.target.value),0,1)})}/></label><label className="text-[9px] text-slate-600">Y<input className={fieldClass()} type="number" min="0" max="1" step=".02" value={selectedPip.y} onChange={(e)=>updatePip(selectedPip.id,{y:clamp(Number(e.target.value),0,1)})}/></label></div><p className="text-[9px] leading-5 text-slate-600">يمكنك سحب فيديو PiP مباشرة داخل شاشة المعاينة لتغيير موضعه.</p></div>}{selectedTitle&&<div className="mt-5 space-y-3"><label className="text-[9px] text-slate-600">TEXT<input className={fieldClass()} value={selectedTitle.text} onChange={(e)=>updateTitle(selectedTitle.id,{text:e.target.value})}/></label></div>}{selectedSubtitle&&<div className="mt-5 space-y-3"><label className="text-[9px] text-slate-600">SUBTITLE<input className={fieldClass()} value={selectedSubtitle.text} onChange={(e)=>updateSubtitle(selectedSubtitle.id,{text:e.target.value})}/></label></div>}{selectedImage&&<div className="mt-5 space-y-3"><label className="text-[9px] text-slate-600">OVERLAY OPACITY<input className={fieldClass()} type="number" min="0" max="1" step=".05" value={selectedImage.opacity} onChange={(e)=>updateImage(selectedImage.id,{opacity:clamp(Number(e.target.value),0,1)})}/></label></div>}{error&&<div className="mt-5 rounded-xl border border-rose-300/20 bg-rose-400/10 p-3 text-[10px] leading-5 text-rose-200">{error}</div>}</aside>
        </section>
      </div>
    </main>
  )
